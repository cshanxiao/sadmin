...
def PermissionVerify():
    u'''
    @summary: 权限认证模块
    @note: 此模块会先判断用户是否是管理员（is_superuser为True），如果是管理员，则具有所有权限,
...
def AddPermission(request):
    ...
    return render_to_response('UserManage/permission.add.html', kwvars,
                              RequestContext(request))
...
def EditPermission(request, ID):
    ...
    return render_to_response('UserManage/permission.edit.html', kwvars,
                              RequestContext(request))
